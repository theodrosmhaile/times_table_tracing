...
heatmap_facet <- ggplot(df_heatmaps, 
  ...
  theme_minimal() +
  labs(
    title = "Heatmaps of Mean Accuracy",
    x = "Multiplier",
    y = "Multiplicand"
  ) +
  theme(
...
heatmap_plot <- ggplot(heatmap_data, aes(x = comparison, y = skill, fill = pearsons_r, label = round(pearsons_r, 2))) +
  ...
  geom_text(size = 3) +  # Add Pearson's r as text in the tiles
  scale_fill_gradient2(
...
```{r}
compute_skill_correlation <- function(encounter_list, skill_count, title_label) {
...
# Calculate mean correlations for 3 skills across all encounters
plot_3_overall <- compute_skill_correlation(
  encounter_list = list(s3l2first, s3l2middle, s3l2last),
  skill_count = 3
)

# Calculate mean correlations for 4 skills across all encounters
plot_4_overall <- compute_skill_correlation(
  encounter_list = list(s4l2first, s4l2middle, s4l2last),
  skill_count = 4
)

# Calculate mean correlations for 5 skills across all encounters
plot_5_overall <- compute_skill_correlation(
  encounter_list = list(s5l2first, s5l2middle, s5l2last),
  skill_count = 5
)

# Calculate mean correlations for 6 skills across all encounters
plot_6_overall <- compute_skill_correlation(
  encounter_list = list(s6l2first, s6l2middle, s6l2last),
  skill_count = 6
)

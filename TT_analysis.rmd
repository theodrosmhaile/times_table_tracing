...
ggplot() +
  ...
  geom_point(data = accuracy_by_multiplier,
             aes(x = multiplier, y = mean_accuracy, group = "Multiplier", color = "Multiplier"),
             size = 3) +
  # geom_errorbar(data = accuracy_by_multiplier,
  #               aes(x = multiplier, ymin = mean_accuracy - sd_accuracy, ymax = mean_accuracy + sd_accuracy),
  #               width = 0.2, color = "#4575b4", position = position_dodge(0.1)) +
  # Curve for multiplicands (red)
  ...
  geom_point(data = accuracy_by_multiplicand,
             aes(x = multiplicand, y = mean_accuracy, group = "Multiplicand", color = "Multiplicand"),
             size = 3) +
  # geom_errorbar(data = accuracy_by_multiplicand,
  #               aes(x = multiplicand, ymin = mean_accuracy - sd_accuracy, ymax = mean_accuracy + sd_accuracy),
  #               width = 0.2, color = "#d73027", position = position_dodge(0.1)) +
  # Labels and styling
  labs(
...
# Calculate and plot correlations between skill probabilties from L1 and memory decay (alpha) from L3
...
finishers_s3l1first <- filter_graaftel_data("./GT_ratings/finishers13/s3l1first.csv", s = 3)
finishers_s3l1middle <- filter_graaftel_data("./GT_ratings/finishers13/s3l1middle.csv", s = 3)
finishers_s3l1last <- filter_graaftel_data("./GT_ratings/finishers13/s3l1last.csv", s = 3)

...
export_graaftel_data(dat_level2_finishers_first, 2, "first")
export_graaftel_data(dat_level2_finishers_middle, 2, "middle")
export_graaftel_data(dat_level2_finishers_last, 2, "last")
...
```{r}
# Identify unique students who have solved problems in Level 2
students_level2 <- dat %>%
  filter(level == 2) %>%
  distinct(user_id) %>%
  pull(user_id)
...
# Find students who are in Level 2 but not in Level 3
students_only_level2 <- setdiff(students_level2, students_level3)
# Find students who are in Level 3 but not in Level 2
students_only_level3 <- setdiff(students_level3, students_level2)

# Create dat_level2_finishers by filtering out students who only did Level 2
dat_level2_finishers <- dat_level2 %>%
  filter(!user_id %in% students_only_level2)
...
dat_level2_finishers_first <- filter_encounter(dat_level2_finishers, "first")
dat_level2_finishers_middle <- filter_encounter(dat_level2_finishers, "middle")
dat_level2_finishers_last <- filter_encounter(dat_level2_finishers, "last")
...
combine_alpha_correlation_data <- function(encounter, graaftel_data, encounter_data) {
  ...
  correlations <- correlations %>%
    mutate(level = "Level 2 to Level 3", encounter = encounter)
  return(correlations)
}

# Load GraafTel outputs for Level 2 from the /GT_ratings/finishers/ directory
finishers_s3l2first <- filter_graaftel_data("./GT_ratings/finishers/s3l2first.csv")
finishers_s3l2middle <- filter_graaftel_data("./GT_ratings/finishers/s3l2middle.csv")
finishers_s3l2last <- filter_graaftel_data("./GT_ratings/finishers/s3l2last.csv")
...
alpha_correlation_results <- bind_rows(
  combine_alpha_correlation_data("First", finishers_s3l2first, dat_level3_finishers_first),
  combine_alpha_correlation_data("Middle", finishers_s3l2middle, dat_level3_finishers_middle),
  combine_alpha_correlation_data("Last", finishers_s3l2last, dat_level3_finishers_last)
)
...
ggplot(alpha_correlation_results, aes(x = encounter, y = skill, fill = pearsons_r, label = round(pearsons_r, 2))) +
  ...
  theme_minimal() +
  labs(
    title = "Level 2 Skills vs Level 3 Memory Decay Rates (Alpha) Across Encounters",
    x = "Encounter",
    y = "Skill"
  )
```
...
df_heatmaps <- df_heatmaps %>%
  mutate(
    ...
    multiplicand = as.factor(multiplicand)
  )
...
heatmap_facet <- ggplot(df_heatmaps, 
  ...
  theme(
    axis.text.x = element_text(size = 7),                  # Axis tick label size (x-axis)
    axis.text.y = element_text(size = 7),                  # Axis tick label size (y-axis)
    ...
    plot.title = element_text(size = 20, hjust = 0),     # Title size and centering
...
s3l1last <- filtered_graaftel_data[["s3l1last"]]
...
# 
# s6l3first <- filtered_graaftel_data[["s6l3first"]]
# s6l3middle <- filtered_graaftel_data[["s6l3middle"]]
# s6l3last <- filtered_graaftel_data[["s6l3last"]]
```
...
ggplot(alpha_correlation_results, aes(x = encounter, y = skill, fill = pearsons_r, label = round(pearsons_r, 2))) +
  ...
  theme(
    ...
    axis.title.y = element_text(size = 18),                # Y-axis label size
    legend.key.height = unit(2, "cm"),                     # Legend key height
    plot.title = element_text(size = 16, hjust = 0),     # Title size and centering
    strip.text = element_text(size = 12),                   # Facet label size
    legend.text = element_text(size = 14)
  ) +
  labs(
...
# Factor analysis on skills
skill_data <- s3l2first %>% select(skill1, skill2, skill3)
fa_model <- fa(skill_data, nfactors = 3, rotate = "varimax")
